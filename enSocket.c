...
int enUDPRecv(struct enBuffer* buff, unsigned int *ip, unsigned short *port){
	...
	int recvd = 0;
	int size;
	while((recvd = recvfrom(enUDPSocketOut, buff->data+buff->ptr, maxBuffer, MSG_PEEK, (struct sockaddr *) &addr, &size)) >= maxBuffer){
		maxBuffer <<= 1;
	...
	*port = ntohs(addr.sin_port);
	return recvfrom(enUDPSocketOut, buff->data+buff->ptr, maxBuffer, 0, (struct sockaddr *) &addr, &size);
}
